...
- `workbook_inlineStr_edgecases.pptx`: Bar chart workbook uses inlineStr rich-text runs and whitespace; extraction should preserve text.
...
- `line_chart_cached_values_missing.pptx`: Line chart workbook contains formula cells missing cached <v>; missing numeric values should follow policy.
...
- `mix_write_bar_line_single_axis_variantA.pptx`: Mixed bar+line chart with shared axes and reordered plot elements; used for write-path robustness.
...
- `mix_write_secondary_axis_valid_variantA.pptx`: Secondary-axis mix with reordered axis/plot nodes; valid write-path case.
- `mix_write_secondary_axis_valid_variantB.pptx`: Secondary-axis mix with different series counts across plots; categories shared.
