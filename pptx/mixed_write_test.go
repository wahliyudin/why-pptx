...
func TestMixedApplyChartDataSecondaryAxisBestEffort(t *testing.T) {
	input := fixturePath("mix_write_secondary_axis_invalid_axis_group.pptx")
	output := filepath.Join(t.TempDir(), "output.pptx")
	...
	alerts := doc.AlertsByCode("WRITE_MIX_AXIS_GROUP_INVALID")
	if len(alerts) != 1 {
		t.Fatalf("expected WRITE_MIX_AXIS_GROUP_INVALID alert, got %d", len(alerts))
	}
...
func TestMixedApplyChartDataSecondaryAxisMismatchedCategoriesBestEffort(t *testing.T) {
	input := fixturePath("mix_write_secondary_axis_mismatched_categories.pptx")
	output := filepath.Join(t.TempDir(), "output.pptx")
...
func TestMixedPostflightSecondaryAxisInvalidCacheStrict(t *testing.T) {
	input := fixturePath("mix_write_secondary_axis_cache_invalid.pptx")
	output := filepath.Join(t.TempDir(), "output.pptx")
...
func TestMixedPostflightSecondaryAxisInvalidGroupStrict(t *testing.T) {
	input := fixturePath("mix_write_secondary_axis_invalid_axis_group.pptx")
	output := filepath.Join(t.TempDir(), "output.pptx")
	...
	alerts := doc.AlertsByCode("POSTFLIGHT_MIX_SECONDARY_AXIS_INVALID")
	if len(alerts) != 1 {
		t.Fatalf("expected POSTFLIGHT_MIX_SECONDARY_AXIS_INVALID alert, got %d", len(alerts))
	}
